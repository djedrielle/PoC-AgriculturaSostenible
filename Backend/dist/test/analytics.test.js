...
const production_1 = require("../src/Business/Models/production");
...
describe('Production Endpoints', () => {
    ...
    describe('GET /analytics/getProductionHistory', () => {
        it('should return production history for valid user_id', async () => {
            ...
            const mockProductionHistory = mockProductionData.map(data => new production_1.Production(data.location, data.user_id, data.crop_type, data.crop_variety, data.est_harvest_date, data.amount, data.measure_unit, data.biologic_features, data.agro_conditions, data.agro_protocols, data.active, data.contract_id));
